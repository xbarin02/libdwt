...
int main()
{
	...
	const int stride_y = sizeof(float), stride_x = dwt_util_get_opt_stride(stride_y * x);
	...
	for(int jj = 1; jj <= j; jj++)
	{
		...
		dwt_util_subband_s(data1, stride_x, stride_y, x, y, x, y, jj, DWT_LH, &subband_ptr, &subband_size_x, &subband_size_y);
		...
		for(int yy = 0; yy < subband_size_y; yy++)
		{
			...
			{
				float *coeff = dwt_util_addr_coeff_s(subband_ptr, yy, xx, stride_x, stride_y);
