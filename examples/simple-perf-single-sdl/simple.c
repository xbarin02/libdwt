...
int main()
{
	...
	// forward transform
	dwt_cdf97_2f_inplace_sdl_s(data1, stride_x, stride_y, x, y, x, y, &j, 0, 0);
	...
	// inverse transform
	dwt_cdf97_2i_inplace_s(data1, stride_x, stride_y, x, y, x, y, j, 0, 0); // FIXME
	...
	dwt_util_perf_cdf97_2_inplace_sdl_s(
		...
		x, y, x, y,
		j, 0, 0,
		1, 20, //4, 8,		// 4 transforms per test loop, 8 test loops
		type,
		&fwd_secs,
		&inv_secs);
