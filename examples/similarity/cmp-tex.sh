...
function compare()
{
	...
	echo -e "\t\t\t${TEST}\t& ${WAVELET}\t& {${PRE[PSNR]}${PSNR}}\t\t& {${PRE[SSIM]}${SSIM}}\t\t& {${PRE[PATCHES]}${PATCHES}} \\\\\\\\"
}
...
echo -e "\\\\begin{table}"
echo -e "\t\\\\centering"
echo -e "\t\\\\begin{tabular}{|l|l|r|r|r|}"
echo -e "\t\t\hline"
echo -e "\t\t\t{\\\\bf image} & {\\\\bf interpolation} & {\\\\bf PSNR} & {\\\\bf SSIM} & {\\\\bf PBC} \\\\\\\\"
echo -e "\t\t\hline"
